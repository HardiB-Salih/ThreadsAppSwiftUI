...
struct ThreadReplyService {
    static func uploadThreadReply(_ reply: ThreadReply, forThread thread: Thread) async throws {
    ...
    static func fetchThreadReplies(forThread thread: Thread) async throws -> [ThreadReply] {
        ...
        let threadReplies = snapshot.documents.compactMap({ try? $0.data(as: ThreadReply.self)})
    ...
    static func fetchThreadRepliesCurrentUser(forUser user: User) async throws -> [ThreadReply] {
        let snapshot = try await FirestoreConstants
            .RepliesCollection
